...
First up, let's load the tidyverse package and read in the data. [^1] 
```{r}
...
There are 3 main columnms we'll be working with. The *location* column includes countries, the *subject* includes 4 types of meat consumed, and the *measurement* column contains two types of measurement values for meat consumption.  

Now, let's tidy the data to make it easier to work with.
```{r}
# lowercase column names
colnames(meat) <- tolower(colnames(meat))
```

It would be impractical to try and visualize the entire dataset with a marimekko chart. Instead, let's analyze the meat consumption patterns of 4 countries. We'll examine the data in terms of kilograms per capita. Along with USA, I'll pick 3 countries I've visited in recent years: Peru (PER), Japan (JPN), and New Zealand (NZL). 
...
In the plot above, we can see we have 5 columns each representing a different country. Within each column, we have the different meat categories. On both the x and y axes, the boxes are scaled to the proportion as a percentage of the overall total consumption of meat type (x-axis) or country (y-axis).
...
* Sheep is the least consumed meat overall. New Zealand consumes the most sheep meat in kg/capita compared to the other countries and accounts for 18.26% of the countries overall meat consumption.  
* Pig is consumed most prominently in Japan accounting for 41.5% of its total meat consumption. Japan also has the highest proportion of pig consumption compared to the other countries. 
...
* In terms of total meat consumption, USA is the largest consumer of the 4, followed by New Zealand. 

While it is possible to gather useful information from this chart, there are still some challenges. For instance, we don't know the absolute values since we've plotted the proportion as a percentage. Additionally, it's difficult to compare the width of the boxes to determine which countries consume more meat as a function of kilograms per capita. Japan and Peru seem to be very close as is the case with New Zealand and the USA. 
